...
class TwoGoal2OOMDP(OOMDP):
    ...
    def __init__(self, width, height, agent, walls, goals, slip_prob=0, gamma=0.99, step_cost=0, weights=None, env_code=None, sample_rate=5):
        ...
        OOMDP.__init__(self, TwoGoal2OOMDP.ACTIONS, self._two_goal_transition_func, self._two_goal_reward_func,
    ...
    def _create_state(self, agent_oo_obj):
        ...
        objects = {c : [] for c in TwoGoal2OOMDP.CLASSES}
    ...
    def _two_goal_transition_func(self, state, action):
        ...
        state_is_goal = two_goal_helpers.is_goal_state(self, state)
...
def _error_check(state, action):
    ...
    if action not in TwoGoal2OOMDP.ACTIONS:
...
def main():
    ...
    two_goal_world = TwoGoal2OOMDP(10, 10, agent=agent, walls=[])
