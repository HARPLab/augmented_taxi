...
def obtain_summary(vi_name):
    ...
    # a) single environment with a single weight
    passengers_a = [{"x": 4, "y": 1, "dest_x": 1, "dest_y": 1, "in_taxi": 0}]
    tolls_a = [{"x": 3, "y": 1, "fee": 1}, {"x": 3, "y": 2, "fee": 1}]
    ...
    # b) enumeration of all possible optimal policies from possible environments x weight candidates
    ...
    if n_processed_envs < len(env_codes):
        for env_idx in range(n_processed_envs, len(env_codes)):
            ...
            for wt_candidate in wt_uniform_sampling:
                ...
                # if np.ndarray.tolist(wt_candidate) == np.ndarray.tolist(weights): # todo: debugging
                #     fixed_agent = FixedPolicyAgent(vi_candidate.policy)
                #     mdp_candidate.visualize_agent(fixed_agent)

                trajectory = mdp_helpers.rollout_policy(mdp_candidate, vi_candidate)
                # if np.ndarray.tolist(wt_candidate) == np.ndarray.tolist(weights): # todo: debugging
                #     mdp_candidate.visualize_trajectory(trajectory)
    ...
    n_demonstrations = 10
    bayesian_IRL_summary = bayesian_IRL.obtain_summary(n_demonstrations, weights, wt_uniform_sampling, wt_vi_traj_candidates, visualize=True)
