...
def extract_constraints_policy(args):
    env_idx, data_loc, BEC_depth, step_cost_flag, lock = args
...
def extract_constraints(data_loc, BEC_depth, step_cost_flag, pool, lock, vi_traj_triplets=None, print_flag=False):
    '''
    ...
    # commenting out to prevent multiprocessing issues
    ...
    if vi_traj_triplets is None:
        ...
        args = [(i, data_loc, BEC_depth, step_cost_flag, lock) for i in range(n_envs)]
...
def combine_limiting_constraints_IG(args):
    ...
    mdp_features, trajs_opt, min_BEC_constraints_running, particles, compute_IG_flag, compute_n_diff_constraints_flag, lock = args
    ...
    if variable_filter.dot(mdp_features.T) > 0:
        ...
        # print('min_env_constraints_record: ' + str(min_env_constraints_record))
        # print('info_gains_record_env :', info_gains_record)
        # print('n_diff_constraints_env :', n_diff_constraints)
    ...
    # return info_gains_record, min_env_constraints_record, n_diff_constraints
...
def obtain_remedial_demonstrations(data_loc, pool, particles, n_human_models, BEC_constraints, min_subset_constraints_record, env_record, traj_record, traj_features_record, previous_demonstrations, visited_env_traj_idxs, variable_filter, mdp_features_record, consistent_state_count, weights, step_cost_flag, type='training', info_gain_tolerance=0.01, consider_human_models_jointly=True, n_human_models_precomputed=0, fallback='particle_filter'):
    ...
    if n_human_models_precomputed != 0:
        ...
        while not remedial_demonstration_selected:
            ...
            print('Len min_env_constraints_record for remedial: {}'.format(len(min_env_constraints_record)))
